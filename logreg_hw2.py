...
def main():
  ...
  dum = dummyAugment(X_test)
  test = np.concatenate((np.expand_dims(np.array(range(np.shape(X_test)[0]), dtype = int), axis = 1), dum@w >= 0), axis = 1)
  header = np.array([["id", "type"]])
  test = np.concatenate((header, test))
  np.savetxt('predicted.csv', test, fmt = '%s', delimiter = ',')
...
def calculateNegativeLogLikelihood(X,y,w):
  ...
  for i, z in enumerate(X):
    t_x = np.transpose(w)@np.array(z)
    log = logistic(t_x)
    nll -= (y[i] * np.log(log + .0000001) + (1 - y[i]) * np.log(1 - log + .0000001))
  return nll
